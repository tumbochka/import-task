...
import {
  SalesOrderReportWithSales,
  salesOrderReportExportDataOrderFields,
  salesOrderReportExportDataPopulation,
} from './variables';
...
export const getSalesFullNameFromOrder = (
  order: SalesOrderReportWithSales,
): string => {
  return (
...
export const calculateExportOrderAverageGrossMargin = (
  order,
): number | null => {
...
// csv helpers
export const getSalesReportCSVHeaders = (
  extraColumns: string[],
): string[][] => {
  const csvHeaders = [
    [
...
export const getSalesReportCSVData = (
  order,
  extraColumns: string[],
  dateFormat,
  timeFormat,
  ...
  const itemArr = [
    ...
    order?.tax?.toString() ?? '',
    order?.tip?.toString() ?? '',
    formatToPercentage(calculateExportOrderAverageGrossMargin(order)),
    soldDateString,
    dueDateString,
...
export const getSalesReportCSVString = async (
  orders,
  extraColumns: string[],
  userId,
) => {
  ...
  const headers = getSalesReportCSVHeaders(extraColumns)[0];
  const rows = orders.map((order) =>
    getSalesReportCSVData(order, extraColumns, dateFormat, timeFormat),
  );
...
export const generateSalesOrderReportData = async (job) => {
  const orders = await strapi.entityService.findMany('api::order.order', {
    ...
    pagination: { limit: -1 },
    fields: salesOrderReportExportDataOrderFields as any,
    populate: salesOrderReportExportDataPopulation as any,
  });
...
export const generateSalesOrderReportCSV = async (job) => {
  const extendedOrders = await generateSalesOrderReportData(job);

  return getSalesReportCSVString(
    extendedOrders,
