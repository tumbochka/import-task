...
import fastUpdateContactQueue from './../../../../../src/api/redis/bullmq/fastUpdateContact';
import { NexusGenInputs } from './../../../../../src/types/generated/graphql';
import {
  CONTACTS_IMPORT_IDENTIFIER,
  FAST_UPDATE_CONTACT_IDENTIFIER,
...
import { singleContactUpdating } from './../helpers/importing/utils/helpers/singleContactUpdating';
    ...
    try {
      if (updatableContacts && updatableContacts?.length) {
        for (let i = 0; i < updatableContacts?.length; i++) {
          ...
          if (updatableContact) {
            await singleContactUpdating(updatableContact, {
              tenantFilter,
...
export const fastUpdateAllContactsFromCSV: GraphQLFieldResolver<
  null,
...
> = async (root, { input }, ctx) => {
  ...
  await fastUpdateContactQueue.add(FAST_UPDATE_CONTACT_IDENTIFIER, {
    updatableContacts,
