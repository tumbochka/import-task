...
class RouteResolver {
  ...
  private get isGraphql(): boolean {
    return this.strapiRoutePrefixes().some((prefix) =>
      this.ctx.url.startsWith(prefix),
    );
  }
  ...
  private get isStatic(): boolean {
    return (
      new RegExp(/[0-9a-z_-]+\.[a-z0-9]{2,5}/i).test(this.ctx.path) ||
      this.ctx.method.toLowerCase() !== 'get'
    );
  }
  ...
  private strapiRouteToPrefix(route: Server.Route): string {
    return route.opts.prefix?.length > 1
      ? route.opts.prefix
      : `/${route.path.split('/')[1]}`;
  }
