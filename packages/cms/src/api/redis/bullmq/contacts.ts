...
import { CONTACTS_IMPORT_IDENTIFIER } from './../helpers/variables/importingVariables';
import { redisConfig } from './../redis';

const contactsImportFieldsQueue = new Queue(CONTACTS_IMPORT_IDENTIFIER, {
  connection: redisConfig,
});
contactsImportFieldsQueue.on('error', (err) => {
  handleLogger('info', 'Redis', 'Redis client error', NOT_DEVELOPEMENT);
});

const queueEvents = new QueueEvents(CONTACTS_IMPORT_IDENTIFIER, {
  connection: redisConfig,
});
...
export default contactsImportFieldsQueue;
