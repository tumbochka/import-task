...
import { FAST_UPDATE_PRODUCT_IDENTIFIER } from './../helpers/variables/importingVariables';
import { redisConfig } from './../redis';

const fastUpdateProductQueue = new Queue(FAST_UPDATE_PRODUCT_IDENTIFIER, {
  connection: redisConfig,
});
fastUpdateProductQueue.on('error', (err) => {
  handleLogger('info', 'Redis', 'Redis client error', NOT_DEVELOPEMENT);
});

const queueEvents = new QueueEvents(FAST_UPDATE_PRODUCT_IDENTIFIER, {
  connection: redisConfig,
});
...
export default fastUpdateProductQueue;
