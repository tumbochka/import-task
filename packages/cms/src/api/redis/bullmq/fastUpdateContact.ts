...
import { FAST_UPDATE_CONTACT_IDENTIFIER } from './../helpers/variables/importingVariables';
import { redisConfig } from './../redis';

const fastUpdateContactQueue = new Queue(FAST_UPDATE_CONTACT_IDENTIFIER, {
  connection: redisConfig,
});
fastUpdateContactQueue.on('error', (err) => {
  handleLogger('info', 'Redis', 'Redis client error', NOT_DEVELOPEMENT);
});

const queueEvents = new QueueEvents(FAST_UPDATE_CONTACT_IDENTIFIER, {
  connection: redisConfig,
});
...
export default fastUpdateContactQueue;
