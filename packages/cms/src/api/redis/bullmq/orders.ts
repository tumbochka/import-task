...
import { ORDERS_IMPORT_IDENTIFIER } from './../helpers/variables/importingVariables';
import { redisConfig } from './../redis';

const ordersImportFieldsQueue = new Queue(ORDERS_IMPORT_IDENTIFIER, {
  connection: redisConfig,
});
ordersImportFieldsQueue.on('error', (err) => {
  handleLogger('info', 'Redis', 'Redis client error', NOT_DEVELOPEMENT);
});

const queueEvents = new QueueEvents(ORDERS_IMPORT_IDENTIFIER, {
  connection: redisConfig,
});
...
export default ordersImportFieldsQueue;
